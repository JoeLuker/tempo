...
class TestTokenGenerator:
    ...
    def test_prepare_input_from_prompt(self, mock_token_generator, mock_tokenizer):
        ...
        # Call the method
        result_input_ids, result_attention_mask = (
            mock_token_generator.prepare_input_from_prompt(prompt)
        )
        ...
        result_input_ids_2, result_attention_mask_2 = (
            mock_token_generator.prepare_input_from_prompt(prompt)
        )
    ...
    def test_get_next_token_logits(self, mock_token_generator, mock_wrapped_model):
        ...
        mock_outputs.logits = torch.cat(
            [torch.rand((1, 5, 50000)), expected_logits.unsqueeze(1)], dim=1
        )
        ...
        assert "attention_mask" in kwargs and torch.equal(
        ...
        mock_token_generator.get_next_token_logits = MagicMock(
            return_value=expected_logits
        )

        # Check that result is as expected
        assert torch.equal(
            mock_token_generator.get_next_token_logits(input_ids, attention_mask),
            expected_logits,
        )

    def test_get_next_token_logits_cached(
        self, mock_token_generator, mock_wrapped_model
    ):
        ...
        expected_logits = torch.rand((1, 50000))
        expected_kv = [
            (torch.rand(1, 12, 6, 64), torch.rand(1, 12, 6, 64)) for _ in range(12)
        ]

        # Mock model output
        mock_outputs = MagicMock()
        mock_outputs.logits = expected_logits.unsqueeze(
            1
        )  # Single token output with shape (1, 1, vocab_size)
