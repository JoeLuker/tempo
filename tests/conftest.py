...
def mock_tokenizer():
    ...
    tokenizer.convert_tokens_to_ids.side_effect = lambda tokens: [
        token_to_id_map.get(t, 100) for t in tokens
    ]
    tokenizer.convert_ids_to_tokens.side_effect = lambda ids: [
        id_to_token_map.get(i, "[UNK]") for i in ids
    ]
...
def mock_model():
    ...
    model.generate.return_value = torch.tensor(
        [[101, 2023, 2003, 1037, 4937, 102, 2001, 2004, 2005]]
    )
...
def mock_wrapped_model(mock_model, mock_tokenizer):
    ...
    def forward(*args, **kwargs):
        ...
        processed_kwargs = {
            k: wrapped_model._ensure_device(v) for k, v in kwargs.items()
        }
...
def mock_token_generator(mock_wrapped_model, mock_tokenizer, mock_device):
    ...
    with patch.object(TokenGenerator, "_setup_logger"):
        token_generator = TokenGenerator(
            mock_wrapped_model, mock_tokenizer, mock_device
        )
