...
class MCTSNode:
    ...
    def add_child(self, token_id, prob, state):
        """Add a child node."""
        self.children[token_id] = MCTSNode(
            parent=self, token_id=token_id, prob=prob, state=state
        )
    ...
    def ucb_score(self, parent_visit_count, c_puct):
        ...
        # Exploration term
        exploration = (
            c_puct * self.prob * math.sqrt(parent_visit_count) / (1 + self.visit_count)
        )
...
class MCTSGenerator:
    ...
    def _simulate(self, node, input_ids, max_steps=5):
        ...
        for _ in range(max_steps):
            ...
            next_token_tensor = torch.tensor(
                [[next_token]], device=sim_input_ids.device
            )
        ...
        for _ in range(self.num_simulations):
            ...
            if not selected_node.expanded():
                ...
                selected_input_ids = torch.tensor(
                    [selected_node.state], device=self.device
                )
    ...
    def generate(
        ...
        # Set up progress bar
        progress_bar = tqdm(
            range(max_tokens), desc="Generating with MCTS", unit="token"
        )
