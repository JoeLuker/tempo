...
class ParallelGenerator(LoggingMixin):
    ...
    def __init__(
        ...
        if use_custom_rope:
            try:
                ...
                if rope_install_success:
                    print(
                        "Using custom RoPE modifications for parallel token positioning"
                    )
                    if self.debug_mode:
                        self.log(
                            f"RoPE modifier initialized with debug mode {'ENABLED' if self.debug_mode else 'DISABLED'}"
                        )
                else:
                    self.log(
                        "RoPE modifier installation failed, disabling custom RoPE",
                        "warning",
                    )
                    self.rope_modifier = None
        ...
        if self.debug_mode:
            self.log(
                f"AttentionManager initialized with debug mode {'ENABLED' if self.debug_mode else 'DISABLED'}"
            )
            self.log(
                f"TokenSelector initialized with debug mode {'ENABLED' if self.debug_mode else 'DISABLED'}"
            )
            self.log(
                f"TokenGenerator initialized with debug mode {'ENABLED' if self.debug_mode else 'DISABLED'}"
            )
    ...
    def update_sequence_length(self, new_length, callback=None):
        ...
        if new_length > self.sequence_length:
            ...
            assert self.step_count == len(
                self.sequence_length_history
            ), "Step count and history length mismatch"

            # Report changes if debugging
            if self.debug_mode:
                self.log(
                    f"Sequence length updated: {old_length} → {new_length}", "debug"
                )
    ...
    def generate(
        ...
        assert selection_threshold is None or (
            0.0 <= selection_threshold <= 1.0
        ), "selection_threshold must be between 0.0 and 1.0"
        ...
        if isolate_parallel_tokens and (
            not self.use_custom_rope or self.rope_modifier is None
        ):
            self.log(
                "Warning: Disabling isolate_parallel_tokens because RoPE modifier is not available",
                "warning",
            )
            isolate_parallel_tokens = False
            print(
                "Warning: Parallel token isolation disabled - RoPE modifier not available"
            )
        ...
        if self.debug_mode:
            print(
                "Debug mode enabled for this generation run - logs will be written to files in logs/ directory"
            )
        ...
        if system_content:
            ...
            if hasattr(self.tokenizer, "apply_chat_template") and callable(
                ...
                model_type = (
                    getattr(self.model.config, "model_type", "").lower()
                ...
                self.log(
                    f"Applied generic chat template for {model_type if model_type else 'unknown'} model with system content"
                )
        ...
        if not disable_kv_cache:
            try:
                ...
                self.log(
                    f"KV cache primed. Num layers: {len(past_key_values) if past_key_values else 'None'}"
                )
            except Exception as e:
        ...
        for logical_step in tqdm(range(max_tokens), desc="Generating tokens"):
            ...
            if disable_kv_cache:
                ...
                attention_mask_for_logits = torch.ones(
                    (1, past_len + num_new), device=self.device
                )
                kv_cache_for_logits = past_key_values
                self.log(
                    f"Logit Gen: Using KV cache. Input shape: {input_ids_for_logits.shape}. Attention mask shape: {attention_mask_for_logits.shape}. Past len: {past_len}"
                )

            # 2. Get Logits for the *next physical position*
            try:
                # Get next token logits with cache
                next_token_logits, new_past_key_values = (
                    self.token_generator.generate_next_token_with_cache(
                ...
                if not disable_kv_cache:
                    ...
                    if past_key_values:
                        self.log(
                            f"KV Cache updated. New cache len: {past_key_values[0][0].shape[2]}"
                        )
                    else:
                ...
                self.log(
                    f"Error during token generation at logical step {logical_step}: {e}",
                    "error",
                )
            ...
            try:
                ...
                selection_threshold = (
                    selection_threshold  # Explicitly name it for clarity
                )
                ...
                if dynamic_threshold:
                    ...
                    if use_relu:
                        ...
                        if progress < relu_activation_point:
                            ...
                            transition_progress = (progress - relu_activation_point) / (
                                1.0 - relu_activation_point
                            )
                            current_threshold = (
                                selection_threshold
                                + (final_threshold - selection_threshold)
                                * transition_progress
                            )
                    else:
                    ...
                    self.log(
                        f"Dynamic threshold at step {logical_step}: {current_threshold:.4f}"
                    )
                ...
                if use_mcts:
                    ...
                    for _ in range(mcts_simulations):
                        ...
                        simulation_result = self._mcts_simulation(
                            mcts_state, depth=mcts_depth, c_puct=mcts_c_puct
                        )
                        best_tokens.extend(simulation_result)

                    # Select top tokens based on MCTS results
                    token_distribution = self._select_from_mcts_results(
                        best_tokens, selection_threshold
                    )
                else:
                ...
                if not token_distribution:
                    self.log(
                        f"Warning: No tokens above selection threshold {selection_threshold} at step {logical_step}. Falling back.",
                        "warning",
                    )
                    token_distribution, _ = self.token_selector.select_tokens(
                        next_token_logits, threshold=0.0, max_tokens=1
                    )
                    if not token_distribution:
                        self.log(
                            "Critical error: Still no tokens selected. Stopping.",
                            "error",
                        )
                        break

                self.log(
                    f"Selected {len(token_distribution)} candidate tokens for logical step {logical_step} using threshold {selection_threshold}"
                )
                ...
                self.log(
                    f"Error selecting tokens at step {logical_step}: {e}. Logits shape: {next_token_logits.shape}",
                    "error",
                )
                break
            ...
            attention_mask = torch.cat(
                [attention_mask, torch.ones((1, len(T_prime_ids)), device=self.device)],
                dim=1,
            )
            ...
            self.logical_layout.append(
                (logical_step, physical_start_idx, physical_end_idx)
            )
            self.log(
                f"Appended {len(T_prime_ids)} tokens. New physical length: {input_ids.size(1)}"
            )
            self.log(
                f"Logical layout updated: ({logical_step}, {physical_start_idx}, {physical_end_idx})"
            )

            # Update sequence length tracking
            self.update_sequence_length(
                input_ids.size(1) - prompt_tokens, sequence_callback
            )
            ...
            if self.rope_modifier is not None and len(T_prime_ids) > 1:
                ...
                position_mapping = {
                    idx: logical_step
                    for idx in range(physical_start_idx, physical_end_idx + 1)
                }
                self.rope_modifier.register_parallel_positions(position_mapping)
                self.log(
                    f"RoPE configured for parallel set at logical position {logical_step} (physical indices {physical_start_idx}-{physical_end_idx})"
                )
                ...
                if any(tid == self.tokenizer.eos_token_id for tid in T_prime_ids):
                    ...
                    if eos_indices:
                        ...
                        if self.logical_layout[-1][0] == logical_step:
                            self.logical_layout[-1] = (
                                logical_step,
                                physical_start_idx,
                                first_eos_physical_idx,
                            )
                        self.log(
                            f"Truncated sequence at first EOS token (physical index {first_eos_physical_idx})."
                        )
                    break
    ...
    def set_debug_mode(self, enabled: bool = True):
        ...
        if self.debug_mode != enabled:
            ...
            if (
                hasattr(self, "attention_manager")
                and self.attention_manager is not None
            ):
    ...
    def _mcts_simulation(self, state, depth, c_puct):
        ...
        for _ in range(depth):
            ...
            current_state["input_ids"] = torch.cat(
                [current_state["input_ids"], new_token], dim=1
            )
