...
class AttentionManager(LoggingMixin):
    ...
    def _validate_input_shapes(self, input_ids: torch.Tensor, attention_mask: torch.Tensor) -> None:
        """
    ...
    def _register_parallel_positions(self, current_pos: int, num_tokens: int) -> None:
        ...
        if self.debug_mode:
            self.log(f"Registered {num_tokens} parallel tokens at position {current_pos}")
    ...
    def _log_no_kv_cache_results(self, updated_input_ids: torch.Tensor, updated_attention_mask: torch.Tensor) -> None:
        """
    ...
    def _handle_multiple_tokens_with_kv_cache(
        ...
        self.log(f"  First token: ID={first_token}, Text='{self.tokenizer.decode([int(first_token)])}'")
    ...
    def _handle_single_token_with_kv_cache(
        ...
        self.log(f"  New token: ID={token_id}, Text='{self.tokenizer.decode([int(token_id)])}'")
    ...
    def _validate_tensors_for_concatenation(self, input_ids: torch.Tensor, new_tokens_tensor: torch.Tensor) -> None:
        """
    ...
    def update_input_efficiently(
        ...
        if is_kv_cache_disabled:
            ...
            return self._handle_kv_cache_disabled(input_ids, attention_mask, new_token_ids, isolate_tokens)
            ...
            if len(new_token_ids) > 1:
                return self._handle_multiple_tokens_with_kv_cache(
                    input_ids, past_key_values, new_token_ids, past_seq_len, isolate_tokens
                )
            else:
            ...
            return self._handle_no_kv_cache(
                input_ids, attention_mask, new_token_ids, is_kv_cache_disabled, isolate_tokens
            )
