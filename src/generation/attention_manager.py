...
class AttentionManager:
    ...
    def create_parallel_set_input(
        ...
        if input_ids.size(0) != attention_mask.size(0):
            raise ValueError(f"Invariant violation: input_ids batch size ({input_ids.size(0)}) must match attention_mask batch size ({attention_mask.size(0)})")
        
        if input_ids.size(1) != attention_mask.size(1):
            raise ValueError(f"Invariant violation: input_ids sequence length ({input_ids.size(1)}) must match attention_mask sequence length ({attention_mask.size(1)})")
            
        # Invariant: new_token_ids must contain valid integers
        if not all(isinstance(token_id, int) for token_id in new_token_ids):
            raise ValueError("Invariant violation: new_token_ids must contain only integer values")
        ...
        if num_new_tokens > 1:
            ...
            if self.debug_mode:
                self.log(f"Registered parallel token set: pos={current_pos}, size={num_new_tokens}")
    ...
    def update_input_efficiently(
        ...
        if input_ids.dim() < 2:
            raise ValueError(f"Invariant violation: input_ids must have at least 2 dimensions, got {input_ids.dim()}")
        
        if attention_mask.dim() < 2:
            raise ValueError(f"Invariant violation: attention_mask must have at least 2 dimensions, got {attention_mask.dim()}")
        
        if input_ids.size(0) != attention_mask.size(0):
            raise ValueError(f"Invariant violation: batch dimensions must match, got {input_ids.size(0)} vs {attention_mask.size(0)}")
        
        if input_ids.size(1) != attention_mask.size(1):
            raise ValueError(f"Invariant violation: sequence dimensions must match, got {input_ids.size(1)} vs {attention_mask.size(1)}")
        ...
        if is_kv_cache_disabled:
            ...
            for token_id in new_token_ids:
                if not isinstance(token_id, int) or token_id < 0:
                    raise ValueError(f"Invariant violation: Token ID {token_id} is not a valid non-negative integer")
            ...
            if len(new_token_ids) > 1 and self.rope_modifier is not None:
                ...
                if self.debug_mode:
                    self.log(f"Registered {len(new_token_ids)} parallel tokens at position {current_pos}")
            ...
            if str(new_tokens_tensor.device) != str(input_ids.device):
                raise ValueError(f"Invariant violation: Device mismatch between input_ids ({input_ids.device}) and new tokens ({new_tokens_tensor.device})")
        ...
        # Handle KV cache enabled case
            ...
            if not (len(past_key_values) > 0 and isinstance(past_key_values[0], tuple) and len(past_key_values[0]) >= 1):
                raise ValueError("Invariant violation: past_key_values has invalid structure, expected non-empty list of tuples")
            ...
            if len(new_token_ids) > 1:
                ...
                if past_seq_len < 0:
                    raise ValueError(f"Invariant violation: Past sequence length {past_seq_len} must be non-negative")
                ...
                if first_input.size(0) != 1 or first_input.size(1) != 1:
                    raise ValueError(f"Invariant violation: first_input has incorrect shape: {first_input.shape}, expected [1, 1]")
                
                # INVARIANT: first_input must have the right token
                if first_input[0, 0].item() != first_token:
                    raise ValueError(f"Invariant violation: first_input contains incorrect token: {first_input[0, 0].item()}, expected {first_token}")
                ...
                if first_attn.size(1) != past_seq_len + 1:
                    raise ValueError(f"Invariant violation: Attention mask dimension ({first_attn.size(1)}) doesn't match expected size ({past_seq_len + 1})")
                
                # DIAGNOSTIC: Log processing parallel tokens efficiently
                self.log(f"Parallel token processing - handling {len(new_token_ids)} tokens efficiently:")
                self.log(f"  First token: ID={first_token}, Text='{self.tokenizer.decode([first_token])}'")
                ...
                if full_size <= past_seq_len:
                    raise ValueError(f"Invariant violation: full_size ({full_size}) must be greater than past_seq_len ({past_seq_len})")
                ...
                if self.full_attention_mask.size(1) != full_size or self.full_attention_mask.size(2) != full_size:
                    raise ValueError(f"Invariant violation: full_attention_mask has wrong shape: {self.full_attention_mask.shape[1:]}, expected [{full_size}, {full_size}]")
                ...
                position_mapping = {past_seq_len + i: past_seq_len for i in range(len(new_token_ids))}
                ...
                if not all(isinstance(layer, tuple) and len(layer) >= 2 for layer in past_key_values):
                    raise ValueError("Invariant violation: past_key_values does not have the expected structure")
                ...
                if len(kv_seq_lengths) > 1:
                    self.log(f"Warning: Inconsistent sequence lengths in KV cache: {kv_seq_lengths}", level="warning")
                ...
                if str(new_input.device) != str(input_ids.device):
                    raise ValueError(f"Invariant violation: Device mismatch between input_ids ({input_ids.device}) and new token ({new_input.device})")
                ...
                if new_attn.size(1) != past_seq_len + 1:
                    raise ValueError(f"Invariant violation: Attention mask dimension ({new_attn.size(1)}) doesn't match expected size ({past_seq_len + 1})")
                ...
                self.log(f"  New token: ID={new_token_ids[0]}, Text='{self.tokenizer.decode([new_token_ids[0]])}'")
        ...
        # This happens at the start of generation when there's no KV cache yet
            ...
            if input_ids.size(1) > 1 and not is_kv_cache_disabled:
                ...
                self.log(f"Warning: No KV cache available despite being in the middle of generation "
                         f"(at position {input_ids.size(1)}). This might indicate a cache reset.", level="warning")
            ...
            if input_ids.dim() < 2:
                raise ValueError(f"Invariant violation: input_ids must have at least 2 dimensions for concatenation, got {input_ids.dim()}")
            if new_tokens_tensor.dim() < 2:
                raise ValueError(f"Invariant violation: new_tokens_tensor must have at least 2 dimensions for concatenation, got {new_tokens_tensor.dim()}")
            if input_ids.size(0) != new_tokens_tensor.size(0):
                raise ValueError(f"Invariant violation: Batch dimension mismatch: {input_ids.size(0)} vs {new_tokens_tensor.size(0)}")
            ...
            if updated_input_ids.size(1) != input_ids.size(1) + new_tokens_tensor.size(1):
                raise ValueError(f"Invariant violation: Concatenation resulted in unexpected sequence length: "
            ...
            if updated_attention_mask.size(1) != updated_input_ids.size(1):
                raise ValueError(f"Invariant violation: Attention mask length ({updated_attention_mask.size(1)}) "
            ...
            if len(new_token_ids) > 1:
                ...
                current_pos = input_ids.size(1)
                if current_pos < 0:
                    raise ValueError(f"Invariant violation: Current position {current_pos} must be non-negative")
                
                # INVARIANT: Full size must include all tokens
                full_size = updated_input_ids.size(1)
                if full_size != current_pos + len(new_token_ids):
                    raise ValueError(f"Invariant violation: Full size {full_size} doesn't match expected length "
                ...
                if self.full_attention_mask.size(2) != full_size or self.full_attention_mask.size(1) != full_size:
                    raise ValueError(f"Invariant violation: Parallel attention mask has wrong size: "
                ...
                if self.rope_modifier is not None:
                    ...
                    if len(position_mapping) != len(new_token_ids):
                        raise ValueError(f"Invariant violation: Position mapping has {len(position_mapping)} entries, "
                    ...
                    if self.debug_mode and self._parallel_token_count % 10 == 0:
                        self.log(f"Processed {self._parallel_token_count} parallel token sets so far")
            
            # INVARIANT: We should return None for past_key_values when there is no KV cache yet
            if past_key_values is not None:
                self.log("Warning: Unexpected non-None past_key_values", level="warning")
    ...
    def create_causal_mask(self, seq_len: int) -> torch.Tensor:
        ...
        if seq_len <= 0:
            raise ValueError(f"Invariant violation: Sequence length must be positive, got {seq_len}")
    ...
    def set_debug_mode(self, enabled: bool = True):
        ...
        if enabled:
            ...
            print(f"AttentionManager debug mode enabled - logging to file at logs/attention_debug.log")
        else:
