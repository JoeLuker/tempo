...
class CoherencePruningStrategy(PruningStrategy):
    ...
    def __init__(self, model, tokenizer, threshold: float = 0.3, device: str = "mps"):
        """
    ...
    def _get_token_coherence_info(
        ...
        if self.token_generator is not None:
            cached_attention, cached_seq_len = (
                self.token_generator.get_cached_attention()
            )
            if cached_attention is not None:
                ...
                if coherence_scores is not None:
                    if self.debug_mode:
                        print(
                            f"Using cached attention for coherence calculation (seq_len: {cached_seq_len})"
                        )
            ...
            if self.debug_mode:
                print(
                    "WARNING: No cached attention available! Creating fallback coherence scores."
                )
    ...
    def _compute_coherence_from_cached_attention(
        self, cached_attention, input_ids: torch.Tensor, token_ids: List[int]
    ...
    def _get_attention_scores(
        self, input_ids: torch.Tensor, token_ids: List[int]
        ...
        for token_id in token_ids:
            ...
            new_seq = torch.zeros(
                (1, seq_len + 1), dtype=input_ids.dtype, device=self.device
            )
        ...
        coherence_scores = avg_attention.max(dim=1)[
            0
        ]  # Max attention value for each token
