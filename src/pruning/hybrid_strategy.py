...
class HybridPruningStrategy(PruningStrategy):
    ...
    def prune_tokens(
        ...
        if self.current_step < self.diversity_steps:
            pruned_tokens = self.diversity_strategy.prune_tokens(input_ids, parallel_tokens)
        else:
            # Switch to coherence pruning after diversity_steps
            pruned_tokens = self.coherence_strategy.prune_tokens(input_ids, parallel_tokens)
