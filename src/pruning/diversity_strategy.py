...
class DiversityPruningStrategy(PruningStrategy):
    ...
    def __init__(self, model, tokenizer, num_clusters: int = 3, device: str = "mps"):
        """
        ...
        for cluster_id in unique_clusters:
            ...
            if np.any(cluster_mask):
                ...
                pruned_tokens.append(
                    (token_ids[best_idx], parallel_tokens[best_idx][1])
                )
    ...
    def _get_token_embeddings(
        ...
        batch_input_ids = torch.zeros(
            (num_tokens, seq_len + 1), dtype=input_ids.dtype, device=self.device
        )
        ...
        batch_input_ids[:, seq_len] = torch.tensor(
            token_ids, dtype=input_ids.dtype, device=self.device
        )
