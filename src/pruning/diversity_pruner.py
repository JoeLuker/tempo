...
class DiversityPruner:
    ...
    def prune_parallel_tokens(
        ...
        token_tuples = [(int(tid), float(prob)) for tid, prob in zip(token_ids, token_probs)]
    ...
    def print_performance_stats(self):
        ...
        if self.perf_stats['total_tokens'] > 0:
            prune_pct = (self.perf_stats['tokens_pruned'] / self.perf_stats['total_tokens']) * 100
            print(f"  Tokens pruned: {self.perf_stats['tokens_pruned']}/{self.perf_stats['total_tokens']} ({prune_pct:.1f}%)")
