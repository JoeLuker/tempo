...
class RetroactivePruner:
    ...
    def retroactively_prune(
        ...
        try:
            ...
            if self.debug_mode:
                print(
                    f"Retroactive pruning with attention threshold: {self.attention_threshold}"
                )
        ...
        for pos in sorted(all_parallel_tokens.keys()):
            ...
            if abs_pos < len(normalized_attn):
                ...
                if self.debug_mode:
                    ...
                    print(
                        f"  Attention score: {attention_score:.4f}, threshold: {self.attention_threshold}"
                    )
                ...
                if (
                    ...
                    if self.debug_mode:
                        print(
                            f"  Pruned position {pos}: kept only '{self.tokenizer.decode([best_token[0]])}'"
                        )
