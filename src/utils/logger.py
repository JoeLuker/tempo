...
class TempoLogger:
    ...
    def __init__(self, name: str, log_file: Optional[str] = None):
        ...
        self.metrics = {
            "log_count_by_level": {
                level: 0 for level in ["DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"]
            },
    ...
    def _setup_handlers(self, log_file: Optional[str]):
        ...
        if config.logging.enable_file_logging:
            ...
            def header_function():
                return (
                    f"--- Log started at {datetime.now().isoformat()} ---\n"
    ...
    def _log(
        ...
        **kwargs,
        ...
        self.metrics["log_count_by_level"][level_name] = (
            self.metrics["log_count_by_level"].get(level_name, 0) + 1
        )
    ...
    def metrics_report(self):
        ...
        report = {
            ...
            "logs_per_second": sum(self.metrics["log_count_by_level"].values())
            / max(1.0, elapsed),
        }
    ...
    def operation(
        ...
        try:
            ...
            if log_result:
                self._log(
                    level,
                    f"Completed operation: {operation_name} in {elapsed:.3f}s",
            ...
            if log_exception:
                self._log(
                    logging.ERROR,
                    f"Failed operation: {operation_name} after {elapsed:.3f}s - {str(e)}",
                    operation=operation_name,
                    duration=elapsed,
                    status="failed",
