...
class ErrorContext:
    ...
    def current(cls, 
                ...
                stack_depth: int = 2,
                args_filter: Optional[Callable[[str, Any], bool]] = None) -> 'ErrorContext':
        """
...
class TempoError(Exception):
    ...
    def _format_for_logging(self) -> str:
        ...
        parts = [
            f"ERROR [{self.code.value}] ({self.severity.value}): {self.message}"
        ]
        
        if self.context:
            parts.append(f"Location: {self.context.module}.{self.context.function} ({self.context.file_path}:{self.context.line_number})")
...
class ModelError(TempoError):
    ...
    def __init__(self, message: str, code: ErrorCode = ErrorCode.MODEL_LOAD_FAILED, **kwargs):
        super().__init__(message, code=code, severity=ErrorSeverity.ERROR, **kwargs)
...
class TokenizationError(TempoError):
    ...
    def __init__(self, message: str, code: ErrorCode = ErrorCode.TOKENIZE_FAILED, **kwargs):
        super().__init__(message, code=code, severity=ErrorSeverity.ERROR, **kwargs)
...
class GenerationError(TempoError):
    ...
    def __init__(self, message: str, code: ErrorCode = ErrorCode.GENERATION_FAILED, **kwargs):
        super().__init__(message, code=code, severity=ErrorSeverity.ERROR, **kwargs)
...
class ResourceError(TempoError):
    ...
    def __init__(self, message: str, code: ErrorCode = ErrorCode.RESOURCE_EXHAUSTED, **kwargs):
        super().__init__(message, code=code, severity=ErrorSeverity.ERROR, **kwargs)
