...
from src.utils.error_manager import (
    ModelError, ErrorCode, safe_execute
)
from src.utils.exception_handlers import (
    handle_model_errors, handle_exceptions
)
...
def get_device_dtype(device: str = None) -> torch.dtype:
    ...
    if config.model.torch_dtype:
        if config.model.torch_dtype == "float16":
            ...
            logger.warning(f"Unknown dtype in config: {config.model.torch_dtype}, using auto-detection")
            dtype = None
...
@handle_model_errors(error_message="Failed to load model", error_code=ErrorCode.MODEL_LOAD_FAILED)
def load_model(
    model_id: str = None,
    revision: str = None,
    device: str = None,
    ...
    **kwargs
    ...
    # Log loading info
    logger.info(f"Loading model: {model_id} (revision: {revision}) on device: {device} with dtype: {torch_dtype}")
    ...
    if hasattr(model_config, "model_type") and model_config.model_type:
        ...
        if 'qwen' in model_config.model_type.lower():
            if hasattr(model_config, "sliding_window") and model_config.sliding_window is not None:
                logger.info(f"Disabling Qwen sliding window (was {model_config.sliding_window})")
    ...
    model_args = {
        ...
        "low_cpu_mem_usage": kwargs.pop("low_cpu_mem_usage", config.model.low_cpu_mem_usage),
        "trust_remote_code": trust_remote_code,
        "revision": revision,
        **kwargs
    }
...
@handle_model_errors(error_message="Failed to load model components", error_code=ErrorCode.MODEL_LOAD_FAILED)
def load_tempo_components(
    ...
    **kwargs
    ...
    model, tokenizer = load_model(model_id=model_id, device=device, load_tokenizer=True, **kwargs)
    result["model"] = model
...
@handle_exceptions(error_message="Failed to check for quantization support", error_code=ErrorCode.MODEL_LOAD_FAILED)
def check_quantization_support() -> Dict[str, bool]:
    ...
    try:
        ...
        logger.info("bitsandbytes not installed, 4-bit and 8-bit quantization unavailable")
...
@handle_model_errors(error_message="Failed to load quantized model", error_code=ErrorCode.MODEL_LOAD_FAILED)
