...
def handle_model_errors(error_message: str = "Model operation failed", **kwargs):
    ...
    return handle_exceptions(
        error_message=error_message, error_class=ModelError, **kwargs
    )
...
def handle_tokenization_errors(error_message: str = "Tokenization failed", **kwargs):
    ...
    return handle_exceptions(
        error_message=error_message, error_class=TokenizationError, **kwargs
    )
...
def handle_generation_errors(error_message: str = "Text generation failed", **kwargs):
    ...
    return handle_exceptions(
        error_message=error_message, error_class=GenerationError, **kwargs
    )


def handle_initialization_errors(
    error_message: str = "Initialization failed", **kwargs
):
    """
    ...
    return handle_exceptions(
        error_message=error_message, error_class=InitializationError, **kwargs
    )
...
def handle_resource_errors(error_message: str = "Resource operation failed", **kwargs):
    ...
    return handle_exceptions(
        error_message=error_message, error_class=ResourceError, **kwargs
    )
...
def exception_context(
    ...
    try:
        ...
        context.additional_info.update(
            {"operation": operation_name, "duration": f"{elapsed:.3f}s"}
        )
        ...
        logger.error(
            f"Error in operation {operation_name} after {elapsed:.3f}s: {str(error)}"
        )
        raise error from e
